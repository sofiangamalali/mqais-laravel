...
@section('content')
                ...
                <tr>
                    <td colspan="6">
                        <center style="width:100%;font-size: 20px">
                            <b>{{__('lang.There Is No Best Seller Products')}}</b>
                        </center>
                    </td>
                </tr>
            @endif
    ...
    <div id="add-product-box" class="modal">
        <div class="content">
            ...
            <form method="POST" action="{{ route('admin.bestSeller.store') }}" class="actForm" enctype="multipart/form-data">
                @csrf
                <h2>{{__('lang.Add New Best Seller Product')}}</h2>
                ...
                <label for="product_id">{{__('lang.Select Product')}}</label><br>
                ...
                @enderror
                <button class="button-9" role="button" onclick="">{{__('lang.Add As Best Seller')}}</button>
    ...
    <div id="del-product-box" class="modal">
        <div class="content">
            ...
            <form method="POST" action="{{ route('admin.bestSeller.delete') }}" class="actForm">
                @csrf
    ...
    <script>
        ...
        $('#sub_cat_id').click(function() {
            var catId = $('#cat_id').val();
            var subCatId = $('#sub_cat_id').val();
            var lang = $('#lang').val();
            $.ajax({
                type: 'GET',
                url: "getProducts/" + catId + "/"+ subCatId,
                success: function(response) {
                    var options = "";
        ...
        function openDelForm(id, name) {
            ...
            document.getElementById('delHeader').innerHTML += name + " {{__('lang.From Best Seller?')}}";

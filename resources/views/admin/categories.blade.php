...
@section('content')
    ...
    <div class="tablecontainer">
        ...
        <input type="text" id="myInput" onkeyup="myFunction()" placeholder="{{ __('lang.Search for names') }}"
            ...
            @if ($categories != null)
                ...
                @foreach ($categories as $category)
                    <tr>
                        <td>{{ $i }}</td>
                        <td>#{{ $category['id'] }}</td>
                        <td>{{ $category['' . $lang . '_name'] }}</td>
                        <td>
                            <button class="actbtn delbtn" role="button"
                                onclick="openDelForm({{ $category['id'] }},'{{ $category['' . $lang . '_name'] }}')"></button>
                            <button class="actbtn editbtn" role="button"
                                onclick="openEditForm({{ $category['id'] }},'{{ $category['en_name'] }}','{{ $category['ar_name'] }}')"></button>
                            {{-- <button class="actbtn viewbtn" role="button"
                                onclick="ViewCategory({{ $category['id'] }})"></button> --}}
                        </td>
                        @php
                            $i++;
                        @endphp
                ...
                <tr>
                    <td colspan="4">
                        <center style="width:100%;font-size: 20px">
                            <b>{{ __('lang.There Is No Categories') }}</b>
                        </center>
                    </td>
                </tr>
            @endif
        ...
        <button class="button-9" role="button" onclick="openAddForm()">{{ __('lang.Add Category') }}</button>
    ...
    <div id="add-category-box" class="modal">
        <div class="content">
            <a href="#" class="box-close">
                ×
            </a>
            <form method="POST" action="{{ route('admin.category.store') }}" class="actForm">
                @csrf
                <h2>{{ __('lang.Add New Category') }}</h2>
                ...
                @enderror
                <button class="button-9" role="button" onclick="">{{ __('lang.Save Category') }}</button>
    ...
    <div id="edit-category-box" class="modal">
        <div class="content">
            <a href="#" class="box-close">
                ×
            </a>
            <form method="POST" action="{{ route('admin.category.edit') }}" class="actForm">
                @csrf
                <h2>{{ __('lang.Edit Category') }}</h2>
    ...
    <div id="del-category-box" class="modal">
        <div class="content">
            <a href="#" class="box-close">
                ×
            </a>
            <form method="POST" action="{{ route('admin.category.delete') }}" class="actForm">
                @csrf
    ...
    <script>
        ...
        function openAddForm() {
            window.location.href = "#add-category-box";
        }
